...
function HW8() {
  ...
  const finalPeople = people.map((p: UserType) => (
    <div key={p._id}>
      <span className={stl.stlName}>{p.name}</span>
      <span className={stl.stlAge}>{p.age.toString()}</span>
    </div>
  ))
  ...
  return (
    <div>
      ...
      <div style={{paddingTop: '8px'}}>
        ...
        <SuperButton onClick={sortDown} style={{width: "100px"}}>sort down</SuperButton>
